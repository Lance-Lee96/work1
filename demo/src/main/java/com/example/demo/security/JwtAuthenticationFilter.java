...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			//1. 발급된 토큰 읽어오기
			//parseBearer Token() : request에 담긴 토큰을 꺼내는 메서드
			...
			if(token != null && !token.equalsIgnoreCase("null")) {
				...
				//사용자 인증 완료후, SecurityContext 에 인증 정보를 등록
				...
				//request객체에서 인증과 관련된 추가적인 정보를 추출한다.
				...
				//스프링 시큐리티에서 사용자의 인증정보와 Context를 관리하는 중심 클래스
				...
				//인증을 완료한 후, 이 메서드를 사용하여 인증된 사용자 정보를
				//저장할 수 있다.
	...
	//클라이언트가 하는 요청은 request객체에 담긴다.
	private String parseBearerToken(HttpServletRequest request) {
		//Http 요청의 헤더를 파싱해 Bearer토큰을 반환한다.
		String bearerToken = request.getHeader("Authorization");
		
		//Bearer 토큰 형식일 경우 토큰값만 반환
		if(StringUtils.hasText(bearerToken) && bearerToken.startsWith("Bearer ")) {
			return bearerToken.substring(7);//토큰 부분만 잘라서 반환
		}
		return null;
	}
}
